...
- **智能回退修复**：修复 `inputs()` 不存在导致回退静默失败；SP 同步后增加二次校验
- **通知增强**：到货通知含需求量/已配送量/建议再要；库存耗尽时同步更新通知
- **筛选保持**：手动修改状态后保持当前筛选条件
...
| **v5.8.2** | **2026-07-21** | **状态判定策略大重构**：严格按已配送判定；汇总需求改为 sum(max(0,d-t))；自动转入已完成列表；since 修复；14 家 Bug 修复整合 |
| **v5.8.1** | **2026-07-20** | **自动在线升级修复**：Edge Function 时区/库存查询修复；两阶段同步架构；实时配送数据 items 传递；已到货回退去 limit(500) |
