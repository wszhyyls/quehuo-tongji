...
RQZT.dbo.SPFXB_Result（核心缓存表 → 网页直读）
  │
  ├──→ 门店「刷新库存」→ usp_Sync_AllShortageCache → 查 SPFXB_Result（实时，3s）
  │     注：usp_Sync_AllShortageCache 本身不刷新数据，只是从 SPFXB_Result 搬到 Supabase
  │
...
| `usp_AutoDetectOrderStatus_Feedback` | RQZT | 自动检测补货状态变化 | 定时任务 / 管理员同步 |
