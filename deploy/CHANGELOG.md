...
| SP 新写入的"已到货"不会被回退 | 智能回退在 SP 同步之前执行，新写入的不参与回退 | SP 同步后增加二次校验，库存=0 立即回退 |

### 二、通知增强

| 改进 | 通知示例 |
|------|---------|
| 到货通知含需求量 | `020151 仓库有货可配送（本店需求8）` |
| 含已配送量+建议再要 | `020151 仓库有货可配送（需求8，已配2，建议再要6）` |
| 库存耗尽通知同步 | `020151 仓库库存已耗尽，已回退为待处理` |

### 三、筛选保持

- 手动修改商品状态后保持当前筛选条件（状态筛选/编码搜索/供货商）
