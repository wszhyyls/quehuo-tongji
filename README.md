...
## 🔥 v5.5.0 核心升级（2026-06-27）

### 自动检测补货状态 — 精准化重构
| 项目 | v3.x 旧版 | v5.5 新版 |
|------|----------|----------|
| 数据源 | SPFXB_Result 快照 | **ZHYYLS 直查实时库存** |
| 门店库存 | `SPJXB_Result.库存数量` | `ZHYYLS.GoodsStocks(qty)` |
| 在途 | 无 | `EXEC Gp_SendDoing`(30天) |
| 仓库库存 | SPFXB 汇总 | `krec='3'` 直查 |
| 判定逻辑 | 全局汇总/任一满足 | **全部上报门店满足才完成** |
| 仓库判定 | 无 | 仓库库存>订货数量→完成 |
| 连接稳定性 | 端口1290/超时30s | 端口1311/超时60s |

### 判定条件
- 检测范围: 所有非 `已完成`/`厂家断货` 状态（TOP 50）
- 完成条件: `仓库库存 > 订货数量` 或 `全部上报门店的(库存>订货 或 在途>订货)`
- 多门店: 所有上报门店都必须满足，任一门不满足即不完成
...
门店客户端 (Electron/login.html → store.html) 
    ↓ Edge Function (query-shortage-data: 48个action, 3600行)
    ↓ SQL Server RQZT (读写) + ZHYYLS (只读源数据, GoodsStocks+Vptype+Gp_SendDoing) + Supabase (缓存/认证)
管理后台 (admin.html) → 一键同步 → 自动检测补货状态
管理后台 (admin.html)
```
