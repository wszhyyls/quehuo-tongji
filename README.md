...
| 数据同步 | 同步采购计划（SPFXB_Result）、同步商品缓存（Supabase）、自动检测状态 |
...
```
ZHYYLS 源系统（Vptype + GoodsStocks + Vsalebill）
  ↓ usp_Sync_AllShortageCache 存储过程（计算汇总）
RQZT.dbo.SPFXB_Result（中间缓存表）
  ↓ sync_with_auto_status（管理员触发）
Supabase shortage_storestock_cache（Supabase 缓存）
  ↓ get_store_inventory / get_product_detail
门店前端 / 管理后台（展示）
```

**刷新库存捷径**：门店端点击「刷新库存」→ 直接同步 SPFXB_Result + 查 SQL Server（绕过 Supabase 缓存）
...
| 存储过程 | 用途 |
|------|------|
| `usp_Sync_AllShortageCache` | 全量同步 SPFXB_Result（从源系统计算汇总） |
| `usp_Sync_Shortage_ZHYYLS_Only` | 仅同步 ZHYYLS 实时数据 |
| `usp_GetPurchasePlanWithFeedback` | 采购计划查询（含订货反馈状态） |
| `usp_UpdateActualOrder` | 设置实际订货数量 → 自动改为"已订购" |
| `usp_AutoDetectOrderStatus_Feedback` | 自动检测补货状态变化 |
| `usp_GetSyncLog` | 获取同步日志 |
