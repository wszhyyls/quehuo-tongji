...
> **适用版本**：v5.8.3 | **更新日期**：2026-08-10  
> **项目名称**：WSZH-ShortageStore | **所属**：微山县众和医药连锁有限公司

---

## 🔥 v5.8.3 核心升级（2026-08-10）
...
| Bug | 根因 | 修复 |
|-----|------|------|
| "已到货"商品仓库库存=0，状态未回退 | `pool.request().inputs()` 在 `node-mssql` 中不存在，抛 TypeError 被静默吞掉 | 改用 `req.input(name, type, value)` 逐个添加参数 |
| SP 新写入的"已到货"不会被回退 | 智能回退在 SP 同步之前执行，SP 同步后新写入的不参与回退 | SP 同步完成后增加二次校验，库存=0 的立即回退为"待处理" |

#### 2. 通知内容增强

| 通知类型 | 消息示例 |
|---------|---------|
| 到货（无配送） | `020151 仓库有货可配送（本店需求8）` |
| 到货（有配送） | `020151 仓库有货可配送（需求8，已配2，建议再要6）` |
| 库存耗尽 | `020151 仓库库存已耗尽，已回退为待处理` |

> 通知使用 `upsert` + `onConflict: 'store_id,product_code'`，同一门店同一商品只有一条通知，状态变化时自动覆盖。

#### 3. 筛选状态保持

手动修改商品状态（如下拉框改状态）后，保持当前筛选条件（状态筛选/编码搜索/供货商），不再重置。
...
门店端通知中心（v5.7） ← 实时推送
```
- **一键同步**：商品缓存同步 + `preciseAutoDetectStatus`（SP判定） + 智能回退（同步前）+ 二次校验（同步后）→ 写 `store_notifications`（含需求量/已配送量/建议再要）→ Realtime 推送门店
- **判定顺序**：门店配送满足（已完成）→ 仓库/采购满足（已到货）→ 其他（待处理）
- **智能回退**：仓库库存=0 的"已到货"自动回退为"待处理"，同步更新通知为"库存已耗尽"
...
门店端                                  管理端
  ...
  │                                       ├─ 修改补货状态（待处理→已订购→已到货）
  │                                       ├─ 一键同步（自动判定+智能回退）
  │  ◀──────── 通知中心 (Realtime) ────────┤  ├─ 推送"已到货"通知（含需求量/已配送量/建议再要）
  │                                       │  ├─ 库存耗尽自动更新通知
  │                                       └─ Excel VBA 批量回写
  └─ 查看上报历史 + 审批通知     │
```
...
| 通知中心 | 铃铛按钮 + 弹窗 + Supabase Realtime 实时推送（v5.7），全员互通，含需求量/已配送量/建议再要（v5.8.3） |
| 需求明细 | 后台管理查看各门店上报明细，含实时配送/库存/在途数据（v3.19） |
| 状态变更日志 | 按商品查询历史状态变更记录（v3.19） |
| 智能回退 | "已到货"商品仓库库存=0时自动回退"待处理"，同步更新通知（v5.8.3修复） |
...
| 版本 | 日期 | 主要变更 |
|------|------|----------|
| **v5.8.3** | **2026-08-10** | **智能回退修复 + 通知增强 + 筛选保持**：修复 `inputs()`→`input()` 导致回退静默失败；SP 同步后增加二次校验；通知增强（需求量/已配送量/建议再要/库存耗尽）；手动修改状态后保持筛选条件 |
